...
def trim_video(input_path, output_path, threshold=5, frame_skip=5):
    ...
    while True:
        ...
        if prev_frame is not None:
            ...
            # Only write frames with significant activity
...
def extract_features_from_video(video_path):
    ...
    while True:
        ...
        closest_person_idx = 0  # Default to the first person
        if len(results) > 1:
            distances = [np.linalg.norm(result.boxes.xyxy.cpu().numpy()[0][:2]) for result in results]
            closest_person_idx = np.argmin(distances)

        keypoints = results[closest_person_idx].keypoints.xy.cpu().numpy().flatten()
        if len(keypoints) == 0:
            continue

        bbox = results[closest_person_idx].boxes.xyxy.cpu().numpy()[0]
    ...
    return aggregated_features.reshape(1, -1), None
...
def make_prediction(model, video_path):
    ...
    return f"Predicted Score: {rounded_prediction:.2f} out of 5"
